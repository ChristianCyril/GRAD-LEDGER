...
async function main() {
 ...
 const students = [
  ...
  for (const s of students) {
    await prisma.student.create({
      data: {
        ...
        academic_records: {
          create: {
            record_id:          `REC${s.student_id.slice(3)}`,
        ...
        portal_credential: {
          create: {
            email: `${s.first_name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z]/g, '')}.${s.last_name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z]/g, '')}@gmail.com`,
  ...
  console.log('Seed completed: 42 students, 1 admin.')
}
